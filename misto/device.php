...
class device
{
	...
	protected static $detectionRules;
	protected static $userAgent = null;
	protected static $accept = null;
	// Assume the visitor has a desktop environment.
	protected static $isMobile = false;
	protected static $isTablet = false;
	protected static $phoneDeviceName = null;
	protected static $tabletDevicename = null;
	protected static $operatingSystemName = null;
	protected static $userAgentName = null;
	...
	function __construct()
	{
		...
		self::$detectionRules = array_merge(self::$phoneDevices, self::$tabletDevices, self::$operatingSystems, self::$userAgents);
		self::$userAgent      = isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : null;
		self::$accept         = isset($_SERVER['HTTP_ACCEPT']) ? $_SERVER['HTTP_ACCEPT'] : null;

		if (isset($_SERVER['HTTP_X_WAP_PROFILE']) || isset($_SERVER['HTTP_X_WAP_CLIENTID']) || isset($_SERVER['HTTP_WAP_CONNECTION']) ||
			isset($_SERVER['HTTP_PROFILE']) || isset($_SERVER['HTTP_X_OPERAMINI_PHONE_UA']) || // Reported by Nokia devices (eg. C3)
			isset($_SERVER['HTTP_X_NOKIA_IPADDRESS']) || isset($_SERVER['HTTP_X_NOKIA_GATEWAY_ID']) || isset($_SERVER['HTTP_X_ORANGE_ID']) ||
			isset($_SERVER['HTTP_X_VODAFONE_3GPDPCONTEXT']) || isset($_SERVER['HTTP_X_HUAWEI_USERID']) || isset($_SERVER['HTTP_UA_OS']) ||
			// Reported by Windows Smartphones
			(isset($_SERVER['HTTP_UA_CPU']) && $_SERVER['HTTP_UA_CPU'] == 'ARM') // Seen this on a HTC
		) {
			self::$isMobile = true;
		} elseif (!empty(self::$accept) && (strpos(self::$accept, 'text/vnd.wap.wml') !== false || strpos(self::$accept,
	 ...
	 * @return boolean|null
	 */
	private static function _detect($key = '')
	{
		if (empty($key)) {
			// Begin general search.
			foreach (self::$detectionRules as $_regex) {
				if (empty($_regex)) {
					continue;
				}
				if (preg_match('/' . $_regex . '/is', self::$userAgent)) {
					self::$isMobile = true;
			...
			$key    = strtolower($key);
			$_rules = array_change_key_case(self::$detectionRules);

			if (array_key_exists($key, $_rules)) {
				if (empty($_rules[$key])) {
					return null;
				}
				if (preg_match('/' . $_rules[$key] . '/is', self::$userAgent)) {
					self::$isMobile = true;
